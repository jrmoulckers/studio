...
| Studio Draft principle tree under `principles/design/**` and `principles/experience/**` | `design-engineer`                       | Concise `STUDIO-<AREA>-NNN` Draft successors validated by `scripts/validate-principles.mjs`. Draft and non-normative until the owner ratifies.       |
...
5. Cite owner-ratified principles when they exist. Agents propose; the repository owner
   alone ratifies.
6. The Studio Draft principle tree in [`design/`](design) and [`experience/`](experience)
   carries the design/UI authority as `STUDIO-<AREA>-NNN` Draft principles. Keep every entry
   `Status: Draft` with `Ratification owner: repository owner`, and validate the tree with
   `pnpm principles:check` before proposing changes. It supersedes no legacy realm file and
   leaves the migration ledger at 0/192.
