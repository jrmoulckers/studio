...
- **A suite of one polarity is satisfied by a constant function of that polarity.** A guard that refuses everything passes every negative test, and the case that would expose it is the boring one nobody writes: the assertion that normal input still succeeds. What makes this hard to see is that it is monotone in the wrong direction — a wall of passing refusal tests reads as _thorough_, so each additional negative case raises confidence and degeneracy together. It therefore cannot be fixed by testing harder, only by testing the other direction once. Wherever a suite's cases all share a sign, add the opposite one first; it is usually a single test and it is the only one that can fail for the right reason.
- **Mutate the test you just wrote, before the ones you inherited.** A new test and any bug inside it share an author, a sitting and a blind spot — an ordering assertion written to catch a regression can be satisfied by that regression, and the person least able to see it is the one who just reasoned their way to the assertion. Mutation treated as a periodic audit of the existing suite misses exactly the window where a test is most trusted and least reviewed: the minutes after it goes green. Break the behaviour a newly-added test names before believing the test, and make that the last step of writing it rather than a later pass. That placement is the point, not a detail of sequencing: it is the only formulation of mutation testing with a moment attached to it. "Audit the suite periodically" names no occasion and so never arrives, whereas "the mutation is the last step of writing the test" is a step inside a procedure someone is already carrying out. A practice that depends on remembering it later is a practice that does not exist.
- **State a mutation result as the discriminating pair, not as a count.** "This mutation fails 4 tests" decays the moment anyone adds a fifth test touching the same predicate — the claim was true when written, goes wrong without anyone editing it, and its wrongness is invisible because nothing re-derives it. What the mutation actually demonstrates is a contrast: _this_ test goes red while _that_ one stays green — an update path never consults drift detection, so update coverage cannot stand in for drift coverage. Stated that way it survives every test added later, because it names the behaviour rather than the tally. A changed count is worse than useless, being ambiguous between two opposite causes — coverage was added, or the guard eroded — and a pair names two assertions where a count names none, so only the pair tells you what to re-check when it moves. Counts are decoration on a claim that is really about which assertions discriminate.
...
- **Mutation is the only check that sees a guard whose removal leaves the code looking normal.** A subtractive boundary — a single `!lockEntry` conjunct separating _the engine may absorb this_ from _the engine may rewrite this_ — is cheap to state and invisible to lose: nothing about the surrounding code reads differently once it is gone, so review has no anomaly to notice and the suite stays green unless some test discriminates the two sides. Correctness that rests on a term being _present_ leaves no trace when the term is deleted, which is the one defect class a reader cannot be made better at catching. Mutate wherever a boundary is carried by a conjunct rather than by a block of code, and treat a surviving mutant as the specification that boundary never got.
...
- **A stale assertion fails loudly; a stale comment is believed** — which makes a comment stating a measured fact the harder case, not the easier one. The same clause applied to a code comment or a PR body is worse than applied to prose in a document, because proximity to the code lends it authority the code does not actually confer. Note the failure mode this defends against is not a claim that was wrong when written: it was true, verified, and correct at the time, and decayed because something else changed. Checking harder at write time cannot catch it, and only a form that re-derives itself can — which is the whole argument for preferring an assertion to a note. Where a comment must record a measurement, say what makes it a measurement (the command, the revision, the date) so its staleness is visible, and say explicitly when a figure was removed on purpose, or the next reader will helpfully restore it.
